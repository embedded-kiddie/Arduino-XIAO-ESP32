...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.pin_cs           =    TFT_CS; //14;  // CSが接続されているピン番号   (-1 = disable)
      cfg.pin_rst          =    TFT_RST; //33;  // RSTが接続されているピン番号  (-1 = disable)
      cfg.pin_busy         =    -1;  // BUSYが接続されているピン番号 (-1 = disable)

      // ※ 以下の設定値はパネル毎に一般的な初期値が設定されていますので、不明な項目はコメントアウトして試してみてください。

      cfg.panel_width      =   TFT_WIDTH; //240;  // 実際に表示可能な幅
      cfg.panel_height     =   TFT_HEIGHT; //320;  // 実際に表示可能な高さ
...
//*
    { // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_max      = TFT_WIDTH-1; //239;  // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 0;    // タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = TFT_HEIGHT-1; //319;  // タッチスクリーンから得られる最大のY値(生の値)
      cfg.pin_int    = TOUCH_IRQ; //38;   // INTが接続されているピン番号
...
// SPI接続の場合
      ...
      cfg.pin_miso = TFT_MISO; //19;     // MISOが接続されているピン番号
      cfg.pin_cs   = TOUCH_CS; // 5;     //   CSが接続されているピン番号
